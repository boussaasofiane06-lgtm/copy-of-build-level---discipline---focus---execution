...
export const adminApi = {
  ...
  refreshMonthlyDigestQueue: (data?: { audience?: string[]; currentMonthOnly?: boolean }) => api.post<{ success: true; queue: MonthlyDigestQueueItem[] }>("/admin/email/monthly-digest/queue/refresh", data || {}).then(r => r.data),
  getMonthlyDigestQueue: (params?: { audience?: string[]; currentMonthOnly?: boolean }) => api.get<MonthlyDigestQueueItem[]>("/admin/email/monthly-digest/queue", { params: { audience: params?.audience?.join(","), currentMonthOnly: params?.currentMonthOnly ? "true" : undefined } }).then(r => r.data),
  ...
  sendMonthlyDigestTest: (email: string, audience?: string[]) => api.post<{ success: true; skipped?: boolean; message?: string }>("/admin/email/monthly-digest/test", { email, audience }).then(r => r.data),
