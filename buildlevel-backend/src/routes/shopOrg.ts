...
for (const [path, table] of [["collections", "shop_collections"], ["trends", "shop_trends"], ["events", "shop_events"], ["recommended-groups", "shop_recommended_groups"]] as const) {
  router.post(`/admin/shop/${path}`, requireAdmin, async (req, res) => {
    try { await ensureShopOrgTables(); const data = entitySchema.parse(req.body); const db = await getDb(); await db.execute(sql`INSERT INTO ${sql.raw(table)} (name, slug, description, imageUrl, icon, displayOrder, enabled, hidden, featured, published) VALUES (${data.name}, ${data.slug || slugify(data.name)}, ${data.description}, ${data.imageUrl}, ${data.icon}, ${data.displayOrder}, ${data.enabled}, ${data.hidden}, ${data.featured}, ${data.published}) ON DUPLICATE KEY UPDATE name = VALUES(name), description = VALUES(description), imageUrl = VALUES(imageUrl), icon = VALUES(icon), enabled = true, hidden = false, published = true, updatedAt = NOW()`); res.json({ success: true }); } catch (e: any) { res.status(400).json({ error: e.message }); }
  });
