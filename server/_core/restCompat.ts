...
export function registerRestCompatRoutes(app: Express) {
  ...
  for (const [path, table] of [["collections", "shop_collections"], ["trends", "shop_trends"], ["events", "shop_events"], ["recommended-groups", "shop_recommended_groups"]] as const) {
    app.post(`/api/admin/shop/${path}`, requireAdminRest, async (req, res) => {
      try { await ensureShopOrgTables(); const data = orgEntitySchema.parse(req.body); const db = await requireDb(); await db.execute(sql.raw(`INSERT INTO ${table} (name, slug, description, imageUrl, icon, displayOrder, enabled, hidden, featured, published) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name), description = VALUES(description), imageUrl = VALUES(imageUrl), icon = VALUES(icon), enabled = true, hidden = false, published = true, updatedAt = NOW()`), [data.name, data.slug || orgSlug(data.name), data.description, data.imageUrl, data.icon, data.displayOrder, data.enabled, data.hidden, data.featured, data.published]); res.json({ success: true }); } catch (e: any) { res.status(400).json({ error: e.message }); }
    });
